...
//This needs to be fixed, because I am still adding an alarm
...
void initialize_thread(int open_position){
    ...
    if ( threads[open_position].uc_stack.ss_sp == 0 ) {
        perror( "malloc: Could not allocate heap\n" );
        exit( 1 );
    }
}
...
void schedule_threads_with_preempt(int usecs) {
    ...
    while(num_threads > 0){
        for(current_thread_index = 0; current_thread_index < MAX_THREADS; current_thread_index++){
            ...
            if(thread_status_done[current_thread_index]){
                free_thread(current_thread_index); //Free the heap of the thread that just completed
            }
        }
    }
